...
import { getFormOptions, insertQuote, trackClick, getSiteSetting } from '@/lib/db';

export default function QuoteModal() {
  const { quoteOpen, setQuoteOpen } = useModal();
  const [contactPhone, setContactPhone] = useState('');
  const [contactWp, setContactWp] = useState('');
    ...
    if (quoteOpen) {
      async function loadOptions() {
        try {
          ...
          setContactPhone(phones[0]?.replace(/[^0-9+]/g, '') || '');
          setContactWp(contact.whatsapp?.replace(/[^0-9]/g, '') || '');
      ...
      <div className="modal-content quote-modal-content">
        ...
        <div className="direct-contact-section">
          ...
          <div className="contact-buttons">
            <a href={`tel:${contactPhone}`} className="contact-btn call-btn" onClick={handleCallClick}>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z" />
              </svg>
              Hemen Ara
            </a>
            <a href={`https://wa.me/${contactWp}`} target="_blank" rel="noopener noreferrer" className="contact-btn whatsapp-btn" onClick={handleWhatsAppClick}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-row">
                <div className="input-group half">
                  <input type="number" value={age} onChange={(e) => setAge(e.target.value)} required min="10" max="100" />
                  <label>Yaş</label>
                  <span className="bar"></span>
                </div>
                <div className="input-group half">
